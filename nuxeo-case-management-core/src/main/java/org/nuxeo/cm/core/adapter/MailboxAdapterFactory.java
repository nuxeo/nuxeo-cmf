...
public class MailboxAdapterFactory implements DocumentAdapterFactory {
     ...
     * {@link MailboxConstants#CASE_FOLDER_FACET}
     *
     * @param doc
     * @throws ClientRuntimeException if document has the required facet but
     *             does not have the required
     *             {@link MailboxConstants#CASE_FOLDER_SCHEMA}
     */
    public static boolean checkDocument(DocumentModel doc) {
        if (!doc.hasFacet(MailboxConstants.CASE_FOLDER_FACET)) {
            // not a mailbox
            return false;
        } else if (!doc.hasSchema(MailboxConstants.CASE_FOLDER_SCHEMA)) {
            throw new CaseManagementRuntimeException(
                    "Document should contain schema "
                            + MailboxConstants.CASE_FOLDER_SCHEMA);
        }
        return true;
    }
