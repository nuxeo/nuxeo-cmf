...
public class ReplySentListener implements EventListener {
    ...
    public void handleEvent(Event event) throws ClientException {
        ...
        MailEnvelope env = dm.getAdapter(MailEnvelope.class);
        if (env == null) {
            return;
        }

        // Retrieve the sent document
        MailEnvelopeItem item = env.getFirstItem(session);
        if (item == null) {
            return;
        }
        ...
        if (itemDoc.hasFacet(CorrespondenceConstants.OUTGOING_CORRESPONDENCE_FACET)
            ...
            DocumentModelList list = session.query("SELECT * FROM Document WHERE ecm:mixinType ='"
                    ...
                    + "' AND"
                    + " cpost:envelopeDocumentId='" + repliedEnvelopeId + "'");
            for (DocumentModel post : list) {
                post.setPropertyValue(CorrespondencePostConstants.IS_ANSWERED,
                        true);
            ...
            if (relationManager == null) {
                log.error("Cannot add reply relation: relation manager is null");
            } else {
                QNameResource docResource = (QNameResource) relationManager.getResource(
                        RelationConstants.DOCUMENT_NAMESPACE, repliedDocument,
                        null);
                QNameResource replyResource = (QNameResource) relationManager.getResource(
                ...
                relationManager.add(RelationConstants.GRAPH_NAME,
