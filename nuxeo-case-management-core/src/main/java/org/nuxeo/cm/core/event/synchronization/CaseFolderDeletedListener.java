 ...
 */
public class CaseFolderDeletedListener extends AbstractSyncCaseFolderListener {

    private static final Log log = LogFactory.getLog(CaseFolderDeletedListener.class);
    ...
    private void deleteDoc(DocumentModel doc, Calendar synchronizeDate, CoreSession session) throws ClientException {
        if (doc.getAllowedStateTransitions().contains(CaseLifeCycleConstants.TRANSITION_DELETE)) {
            ...
            CaseFolder cf = doc.getAdapter(CaseFolder.class);
