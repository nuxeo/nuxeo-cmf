    ...
    public void execute(CoreSession session) {
        try {
            ...
            if (!session.exists(cmfDomainPath)) {
                DocumentModel caseContainer = session.createDocumentModel(CaseConstants.CASE_DOMAIN_PARENT_PATH,
                        CaseConstants.CASE_DOMAIN_NAME, CaseConstants.CASE_DOMAIN_TYPE);
                ...
                // Add new local ACL
                ACE admins = new ACE(SecurityConstants.ADMINISTRATORS, SecurityConstants.EVERYTHING, true);
                ACE admin = new ACE(SecurityConstants.ADMINISTRATOR, SecurityConstants.EVERYTHING, true);
                localACL.add(admins);
                localACL.add(admin);
                // Block inherited ACL
                ACE blockInheritance = new ACE(SecurityConstants.EVERYONE, SecurityConstants.EVERYTHING, false);
            ...
            PathRef casePath = new PathRef(CaseConstants.CASE_DOMAIN_PATH + "/" + CaseConstants.CASE_ROOT_DOCUMENT_NAME);
            if (!session.exists(casePath)) {
                DocumentModel caseContainer = session.createDocumentModel(CaseConstants.CASE_DOMAIN_PATH,
                        CaseConstants.CASE_ROOT_DOCUMENT_NAME, CaseConstants.CASE_ROOT_TYPE);
            ...
            if (!session.exists(mailboxPath)) {
                DocumentModel caseContainer = session.createDocumentModel(CaseConstants.CASE_DOMAIN_PATH,
                        MailboxConstants.MAILBOX_ROOT_DOCUMENT_NAME, MailboxConstants.MAILBOX_ROOT_DOCUMENT_TYPE);
                caseContainer.setPropertyValue(DC_TITLE, "Mailbox");
                session.createDocument(caseContainer);
            }
            // create CMF sections root if missing
            PathRef sectionsPath = new PathRef(CaseConstants.CASE_DOMAIN_PATH + "/sections");
            if (!session.exists(sectionsPath)) {
                DocumentModel caseContainer = session.createDocumentModel(CaseConstants.CASE_DOMAIN_PATH, "sections",
                        "SectionRoot");
